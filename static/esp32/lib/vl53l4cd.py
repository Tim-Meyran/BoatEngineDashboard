...
class VL53L4CD:
    ...
    def __init__(self, i2c, deviceAddress=41):
        self._i2c = i2c
    ...
    def _write_register(self, address, data, length=None):
        ...
        self._i2c.writeto(self._deviceAddress,struct.pack(">H", address) + data[:length])

    def _read_register(self, address, length=1):
        data = bytearray(length)

        b = struct.pack(">H", address)
        self._i2c.writeto(self._deviceAddress,b)
        self._i2c.readfrom(self._deviceAddress,data)
        return data
